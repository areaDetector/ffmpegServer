            ...
            if(frameFinished) {              
                if((ctx == NULL) || (pix_fmt!=pCodecCtx->pix_fmt) || 
                   (width!=pCodecCtx->width) || (height!=pCodecCtx->height))  {    
                    ...
                    printf("width %d height %d pixel format %08x\n", width, height, pix_fmt);
...
ffmpegViewerXv::ffmpegViewerXv (const QString &url, QWidget* parent)
    : QWidget (parent)
{
    init();
    setUrl(url);
...
ffmpegViewerXv::ffmpegViewerXv (QWidget* parent)
    : QWidget (parent)
{
    init();
}

void ffmpegViewerXv::init() {
...
void ffmpegViewerXv::xvsetup()
{
    XvAdaptorInfo * ainfo;
    QX11Info qX11Info = x11Info();
...
void ffmpegViewerXv::paintEvent(QPaintEvent * event)
{
    if(_imw == 0 || _imh == 0)
    {
        return;
    ...
    XvPutImage(dpy, xv_port, w, gc, bmp, 0, 0, _imw, _imh, 0, 0, width(), height());
    XFreeGC(dpy, gc);
}

void ffmpegViewerXv::calcFps() {
...
void ffmpegViewerXv::setX(int x) { 
...
void ffmpegViewerXv::setY(int y) { 
...
// zoom frame
void ffmpegViewerXv::setZoom(int zoom) { 
...
void ffmpegViewerXv::setGx(int gx) {
...
void ffmpegViewerXv::setGy(int gy) {
...
void ffmpegViewerXv::makeCurrent()
{
    //printf("GL not supported\n");
}

void ffmpegViewerXv::updateGL()
{
    //printf("GL not supported\n");
}

// pixel width of gridlines from grid centre outwards
void ffmpegViewerXv::setGs(int gs) {
...
void ffmpegViewerXv::setGrid(bool grid) { 
    if (_grid != grid) { 
        _grid = grid;
...
void ffmpegViewerXv::setUrl(const QString &url) { 
...
void ffmpegViewerXv::initializeGL() {
...
void ffmpegViewerXv::resizeGL (int width, int height) {
    // just set the current viewport to the size of the widget
    glViewport(0, 0, width, height);
    updateViewport();            
}

void ffmpegViewerXv::setReset() {
	ffInit();
}

void ffmpegViewerXv::zoomOntoImage() {
    // zoom onto the image
    makeCurrent();
    disableUpdates = true;
    setX(0);
...
void ffmpegViewerXv::updateImage(int imw, int imh, bool firstImage) 
{
...
// update grid centre
void ffmpegViewerXv::mouseDoubleClickEvent (QMouseEvent* event) {   
...
void ffmpegViewerXv::mousePressEvent (QMouseEvent* event) {   
     if (event->button() == Qt::LeftButton) {
         clickx = event->x();
         oldx = _x;
         clicky = event->y();
...
void ffmpegViewerXv::mouseMoveEvent (QMouseEvent* event) {
...
void ffmpegViewerXv::wheelEvent(QWheelEvent * event) {
    int oldw = _w, oldh = _h;
    // disable automatic updates
    disableUpdates = true;   
    if (event->delta() > 0) {
...
void ffmpegViewerXv::updateViewport() {  
//    initializeGL();
